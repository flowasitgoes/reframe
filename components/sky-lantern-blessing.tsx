...
const LANTERN_POSITIONS = [8, 18, 32, 48, 58, 72, 85, 92];
const LANTERN_DELAYS = [0, 0.6, 1.2, 1.8, 0.4, 1, 1.5, 0.2];
const LANTERN_DURATIONS = [7.5, 9, 8.2, 9.5, 8.5, 9.2, 8, 9.8];
...
const LANTERN_GRADIENTS = [
  ...
  "from-orange-300/95 to-orange-500/90",
  "from-yellow-400/90 to-amber-500/90",
  "from-red-300/90 to-red-500/85",
  ...
  return (
    ...
    >
      ...
      {FIREWORK_POSITIONS.map((pos, i) => (
        <div
          key={i}
          className="firework-burst absolute z-[1]"
          style={{
            left: pos.left,
            top: pos.top,
      ...
      <div className="absolute inset-0 z-[2] pointer-events-none overflow-visible">
        {Array.from({ length: LANTERN_COUNT }).map((_, i) => (
          <div
            key={i}
            className="lantern-rise-item absolute w-10 h-12 bottom-0 pointer-events-none will-change-transform"
            style={{
